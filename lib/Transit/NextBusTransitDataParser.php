...
class NextBusTransitDataParser extends TransitDataParser {
  ...
  private function queryNextBus($params, &$age=null) {
    ...
    if ($cache->isFresh($cacheName)) {
      ...
      $url = Kurogo::getSiteVar('NEXTBUS_SERVICE_URL').http_build_query($params).$specialQueryArgs;
  ...
  private static function getCacheForCommand($params, $daemonMode) {
    ...
    if (!isset(self::$caches[$cacheKey])) {
      ...
      switch ($params['command']) {
        case 'routeList': 
          $cacheKey = 'routes';
          $cacheTimeout = Kurogo::getSiteVar('NEXTBUS_ROUTE_CACHE_TIMEOUT');
          break;
        ...
        case 'predictionsForMultiStops':
          ...
          $cacheTimeout = Kurogo::getSiteVar('NEXTBUS_PREDICTION_CACHE_TIMEOUT');
          break;
          
        case 'vehicleLocations':
          $cacheKey = 'vehicles';
          $cacheTimeout = Kurogo::getSiteVar('NEXTBUS_VEHICLE_CACHE_TIMEOUT');
          break;
      ...
      self::$caches[$cacheKey] = new DiskCache(Kurogo::getSiteVar('NEXTBUS_CACHE_DIR'), $cacheTimeout, TRUE);
